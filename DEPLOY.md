# Deploy runbook (private overlay — `lucien/main` only)

Companion to the generic `MIGRATION.md` (public guide for any migrator) plus
`MIGRATION.lucien.md` (the original first-cutover notes for this maintainer's
deployment). Walks through the first cutover from the pre-fork runtime to
this fork, plus subsequent upgrades.

Live deployment activates the optional `feishu-social` extension by setting
`plugins.entries["openclaw-lark"].config.social.enabled: true` in
`~/.openclaw/openclaw.json` — that key was added by phase 3 of
productionization and is required to keep the previous group-context behavior.
