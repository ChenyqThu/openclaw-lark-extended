...
const reply_in_thread_1 = require("./reply-in-thread.js");
...
async function dispatchToAgent(params) {
    ...
    // Harmonize the fork's replyInThread config (boolean | 'enabled' |
    // 'disabled', with per-group > '*' > account precedence) into upstream's
    // routing input, which only treats `=== true` as enabled. Without this an
    // operator's `'enabled'` string would be ignored by the bot-peer
    // suppression escape hatch and bot→bot replies would be forced to main.
    const replyInThreadConfig = (0, reply_in_thread_1.resolveReplyInThread)(dc.accountScopedCfg?.channels?.feishu, dc.ctx.chatId);
